...
if(isset($_POST['signup'])){
 ...
 try{
   ...
   $udata=$s->prepare('SELECT*FROM userdata WHERE userid=?');
   $udata->execute(array($userid));

   $row=$udata->fetch(PDO::FETCH_ASSOC);
   if($userid==$row['userid']){
      $Emessage="このユーザーIDは既に使用されています、違うユーザーIDを設定してください。";
   }else{
     $s->query("INSERT INTO userdata VALUES('$userid','$name','$year','$month','$day','$mail','$password')");
...
<!--アカウント情報の表示-->
  ...
  <span>パスワード：</span>
  <!--ポートフォリオ用にパスワードは表示-->
  <span><?php echo $password=$_SESSION['keep']['password'] ?></span><br><br>
  <!--
